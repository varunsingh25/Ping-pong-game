...
float arena_half_size_x = 85, arena_half_size_y = 47;
...
simulate_game(Input* input, float dt) {
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		/*Saurish Verma 24BCE10993*/
		draw_number(player_1_score, -10, 40, 1.f, 0xbbffffff);
		draw_number(player_2_score, 10, 40, 1.f, 0xbbffffff);

		// Rendering
		draw_rect(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 0xFFFF00);
		...
		/*Shourya Rawat (24BCE10165)
		*/
		if (hot_button == 0) {
			draw_text("SINGLE PLAYER", -75, -20, .8, 0xff0000);
			draw_text("MULTIPLAYER", 25, -20, .8, 0xaaaaaa);
		}
		else {
			draw_text("SINGLE PLAYER", -75, -20, .8, 0xaaaaaa);
			draw_text("MULTIPLAYER", 25, -20, .8, 0xff0000);
		}

		draw_text("PING PONG", -50, 40, 2, 0xFFA500);
		draw_text("MADE BY  TECH TITANS", -40, 22, .75, 0xffffff);
		draw_text("EESH MISHRA VARUN SINGH", -55, 5, .50, 0xffffff);
		draw_text("SAURISH VERMA SHOURYA RAWAT", -55, 15, .50, 0xffffff);
